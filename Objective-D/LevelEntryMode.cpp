...
void Level1EntryMode::Start() {
	...
	// 카메라 애니메이션 컨트롤러 추가
	auto camCont = scene.AddObject(new CamAnimController(MODE_ENTRY, 1), "cam_anim_cont", LAYER_UI2);
	camCont->SetCameraStartPosition({ 0.f, 40.f, 0.f });
	camCont->SetCameraFovRange(0.f, -50.f);
	camCont->InputCenterPointPosition({ -120.0, 0.0, -120.0 });
}
