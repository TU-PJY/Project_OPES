...
public:
	...
	Texture(ID3D12Device* Device, ID3D12GraphicsCommandList* CmdList, wchar_t* FileName, const WICRect& CropRect, D3D12_FILTER FilterOption= D3D12_FILTER_MIN_MAG_MIP_LINEAR);
	void ReleaseUploadBuffers();
	void Render2D(ID3D12GraphicsCommandList* CmdList);
	void Render3D(ID3D12GraphicsCommandList* CmdList);
	...
	ID3D12Resource* CreateCroppedTextureResourceFromWICFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, wchar_t* pszFileName, ID3D12Resource** ppd3dUploadBuffer, D3D12_RESOURCE_STATES d3dResourceStates, const WICRect& cropRect);
